...
fetch(`${SUPABASE_URL}/rest/v1/nepase_data?select=*&order=trade_date.desc&limit=all`, {
  ...
  .then(data => {
    ...
    data.forEach((row,index) => {
      ...
      tr.innerHTML = `
            ...
            <td>${formatNepaliNumber(row.open_price)}</td>
            <td>${formatNepaliNumber(row.high_price)}</td>
            <td>${formatNepaliNumber(row.low_price)}</td>
            <td>${formatNepaliNumber(row.close_price)}</td>
            <td>${formatNepaliNumber(row.pt_change)}</td>
            <td>${formatNepaliNumber(row.change_p)}</td>
  ...
  .catch(error => {
    ...
    const table = document.getElementById("nepse-table");
    const tr = document.createElement("tr");
