...
// --- INITIALIZATION ---
    function init() {
        ...
        if (savedCollateral) STATE.collateral = parseFloat(savedCollateral);
        ...
        const dom = getDom();
        if (dom.btnBuy) dom.btnBuy.addEventListener('click', () => setTradeMode('buy'));
        if (dom.btnBuy) dom.btnBuy.addEventListener('click', () => setTradeMode('buy'));
        if (dom.btnSell) dom.btnSell.addEventListener('click', () => setTradeMode('sell'));
        if (dom.btnDual) dom.btnDual.addEventListener('click', () => setTradeMode('dual'));

        const buyOrderBtn = document.getElementById('buyOrderBtn');
        if (buyOrderBtn) buyOrderBtn.addEventListener('click', () => submitOrder('buy'));
        const sellOrderBtn = document.getElementById('sellOrderBtn');
        if (sellOrderBtn) sellOrderBtn.addEventListener('click', () => submitOrder('sell'));

        const buyForm = document.getElementById('buyForm');
        if (buyForm) buyForm.addEventListener('submit', (e) => { e.preventDefault(); submitOrder('buy'); });
        const sellForm = document.getElementById('sellForm');
        if (sellForm) sellForm.addEventListener('submit', (e) => { e.preventDefault(); submitOrder('sell'); });

        ['buyPrice', 'buyQty', 'sellPrice', 'sellQty'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.addEventListener('input', () => {
                calculateTotal(id.startsWith('buy') ? 'buy' : 'sell');
                if (id === 'buyPrice') PLVisualizer.update('buy');
            });
        });
        ...
        document.querySelectorAll('.chip[data-action]').forEach(chip => {
            chip.addEventListener('click', () => {
                ...
                if (action === 'addQty') addQty(side, val);
                else if (action === 'setPct') setPct(side, val);
            });
        });

        const buyRRToggle = document.getElementById('buyRRToggle');
        if (buyRRToggle) {
            buyRRToggle.addEventListener('click', () => {
                const container = document.getElementById('buyRRContainer');
                if (container) container.classList.toggle('active');
            });
        }

        const closeHotkey = document.getElementById('closeHotkeyLegend');
        if (closeHotkey) {
            closeHotkey.addEventListener('click', () => {
                const legend = document.getElementById('hotkeyLegend');
                if (legend) legend.style.display = 'none';
            });
        }
        ...
        if (dom.symbolSearch) {
            dom.symbolSearch.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
        ...
        const simInterval = setInterval(simulateTick, 3000);
        
        orderController.signal.addEventListener('abort', () => {
...
// --- SYMBOL CHANGE ---
    ...
    function changeSymbol(newSym) {
        STATE.symbol = newSym;
        STATE.ltp = Math.floor(Math.random() * 2000) + 100;
...
function generateMarketDepth() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const bidRow = domUtils.createElement('tr', {
            events: { click: () => fillPrice(bidPrice) },
            children: [
        ...
        const askRow = domUtils.createElement('tr', {
            events: { click: () => fillPrice(askPrice) },
            children: [
...
// --- MODE HANDLING ---
    function setTradeMode(mode) {
        ...
        [dom.btnBuy, dom.btnSell, dom.btnDual].forEach(b => b && b.classList.remove('active'));

        if (dom.buySection) dom.buySection.style.display = 'none';
        if (dom.sellSection) dom.sellSection.style.display = 'none';
        if (dom.orderCard) dom.orderCard.classList.remove('dual-mode');

        if (mode === 'buy') {
            if (dom.btnBuy) dom.btnBuy.classList.add('active');
            if (dom.buySection) dom.buySection.style.display = 'block';
        } else if (mode === 'sell') {
            if (dom.btnSell) dom.btnSell.classList.add('active');
            if (dom.sellSection) dom.sellSection.style.display = 'block';
        } else {
            if (dom.btnDual) dom.btnDual.classList.add('active');
            if (dom.buySection) dom.buySection.style.display = 'block';
            if (dom.sellSection) dom.sellSection.style.display = 'block';
            if (dom.orderCard) dom.orderCard.classList.add('dual-mode');
        }
    }
...
// --- SUBMISSION ---
    function submitOrder(side) {
        const qtyEl = document.getElementById(`${side}Qty`);
        ...
        if (side === 'buy') {
            ...
            const colEl = document.getElementById('userCollateral');
            if (colEl) colEl.innerText = "NPR " + STATE.collateral.toLocaleString('en-IN', { minimumFractionDigits: 2 });
        }
    ...
    init();
    window.addEventListener('unload', () => orderController.abort());
})();
